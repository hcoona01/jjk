import { c as createLucideIcon, e as commonjsGlobal, g as getDefaultExportFromCjs, r as reactExports, a as clsx, R as React, i as invariant } from "./index-CKsLZ7c0.js";
/**
...
const SCHOOLS = ["SCOPE", "SENSE", "SITE", "SMEC"];
const DEPT_SCHOOL = {
  CSE: "SCOPE",
  IT: "SCOPE",
  "AI-ML": "SCOPE",
...
function rand(min2, max2, decimals = 0) {
  const val = Math.random() * (max2 - min2) + min2;
  ...
  for (const dept of DEPARTMENTS) {
    ...
    for (let i = 0; i < n2; i++) {
      ...
      students.push({
        ...
        placement_status,
        skills,
        placed_at: placement_status === "placed" ? pick(COMPANIES_BY_DEPT[dept]) : void 0,
        package_lpa: placement_status === "placed" ? rand(35, 180) / 10 : void 0,
...
function randomDate(daysBack) {
  ...
  d2.setDate(d2.getDate() - rand(0, daysBack));
  return d2.toISOString().split("T")[0];
}
const mockApplications = Array.from(
  { length: 220 },
  (_, i) => {
    ...
    return {
      ...
      date: randomDate(180),
      role: pick(ROLES),
      package_offered: status === "offered" || status === "hired" ? rand(50, 350) / 10 : void 0
    };
  }
);
const mockJobPostings = mockCompanies.slice(0, 15).flatMap((c2, i) => {
  const dept = c2.departments_hiring[0];
  ...
  return [
    {
      ...
      eligible_branches: c2.departments_hiring,
      positions: c2.open_positions,
      min_cgpa: 6.5 + Math.random() * 1.5,
      package_range: `${(c2.avg_package - 2).toFixed(0)}-${(c2.avg_package + 2).toFixed(0)} LPA`,
    ...
    },
    {
      ...
      eligible_branches: c2.departments_hiring.slice(0, 1),
      positions: Math.ceil(c2.open_positions / 2),
      min_cgpa: 7,
      package_range: `${c2.avg_package.toFixed(0)}-${(c2.avg_package + 4).toFixed(0)} LPA`,
      deadline: randomDate(-rand(5, 30))
    }
  ];
});
mockCompanies.slice(0, 15).map((c2, i) => ({
  id: `PD${String(i + 1).padStart(3, "0")}`,
  company_id: c2.id,
  company_name: c2.name,
  date: randomDate(i < 5 ? -rand(7, 30) : rand(1, 45)),
  department: c2.departments_hiring,
...
const monthlyTrendData = MONTH_LABELS.map(
  (month, i) => ({
    month,
    applications: 15 + Math.floor(Math.sin(i * 0.6) * 8 + Math.random() * 12 + i * 2),
    placements: 4 + Math.floor(Math.sin(i * 0.6) * 4 + Math.random() * 6 + i * 0.8),
    shortlisted: 8 + Math.floor(Math.sin(i * 0.6) * 5 + Math.random() * 8 + i * 1.2)
  })
);
function getDeptStats() {
  return DEPARTMENTS.map((dept) => {
    var _a;
    const students = mockStudents.filter((s2) => s2.department === dept);
    const placed = students.filter((s2) => s2.placement_status === "placed");
    const unplaced = students.filter((s2) => s2.placement_status === "unplaced");
    const inProgress = students.filter(
      (s2) => s2.placement_status === "in-progress"
    );
    const rate = students.length ? placed.length / students.length * 100 : 0;
    const avgCGPA = students.length ? students.reduce((a2, s2) => a2 + s2.cgpa, 0) / students.length : 0;
    const avgAptitude = students.length ? students.reduce((a2, s2) => a2 + s2.aptitude_score, 0) / students.length : 0;
    const avgProg = students.length ? students.reduce((a2, s2) => a2 + s2.programming_score, 0) / students.length : 0;
    const companyCounts = {};
    for (const s2 of placed) {
      if (s2.placed_at)
        companyCounts[s2.placed_at] = (companyCounts[s2.placed_at] ?? 0) + 1;
    }
    const topCompany = ((_a = Object.entries(companyCounts).sort((a2, b2) => b2[1] - a2[1])[0]) == null ? void 0 : _a[0]) ?? "-";
    const skillCounts = {};
    for (const s2 of placed) {
    ...
    const topSkills = Object.entries(skillCounts).sort((a2, b2) => b2[1] - a2[1]).slice(0, 3).map((e3) => e3[0]);
    return {
      department: dept,
      school: DEPT_SCHOOL[dept],
...
function getBatchStats() {
  return BATCHES.map((batch) => {
    const students = mockStudents.filter((s2) => s2.batch === batch);
    const placed = students.filter((s2) => s2.placement_status === "placed");
    const unplaced = students.filter((s2) => s2.placement_status === "unplaced");
    const inProgress = students.filter(
      (s2) => s2.placement_status === "in-progress"
    ...
    const byDept = {};
    for (const dept of DEPARTMENTS) {
      const dStudents = students.filter((s2) => s2.department === dept);
      if (dStudents.length) {
        byDept[dept] = {
          placed: dStudents.filter((s2) => s2.placement_status === "placed").length,
          total: dStudents.length
        };
      }
    }
    const packages = placed.filter((s2) => s2.package_lpa).map((s2) => s2.package_lpa);
    const avgPkg = packages.length ? packages.reduce((a2, b2) => a2 + b2, 0) / packages.length : 0;
    return {
      batch,
      total: students.length,
      placed: placed.length,
      unplaced: unplaced.length,
      in_progress: inProgress.length,
      placement_rate: students.length ? Number.parseFloat(
...
function getTopCompanies(n2 = 10) {
  return mockCompanies.sort((a2, b2) => b2.hired_count - a2.hired_count).slice(0, n2).map((c2) => ({
    name: c2.name,
    tier: c2.tier,
    hired: c2.hired_count,
    avg_package: c2.avg_package,
    departments: c2.departments_hiring
  }));
}
function getSkillGapData() {
  ...
  })).sort((a2, b2) => b2.demand - a2.demand).slice(0, 12);
}
const ACTIVITY_TEMPLATES = [
  (s2, c2) => `${s2.name} applied to ${c2}`,
  (s2, c2) => `${s2.name} received offer from ${c2}`,
  (s2, c2) => `${s2.name} shortlisted at ${c2}`,
  (s2, c2) => `${s2.name} hired by ${c2}`,
  (s2, c2) => `Referral: ${s2.name} → ${c2}`
];
...
function getActivityFeed(limit = 20) {
  return Array.from({ length: limit }, (_, i) => {
    ...
    const time2 = new Date(Date.now() - minutesAgo * 6e4);
    return {
      id: `ACT${i + 1}`,
...
function getPipelineFunnel() {
  ...
  const counts = stages.reduce(
    (acc, s2) => {
      acc[s2] = mockApplications.filter((a2) => a2.status === s2).length;
      return acc;
    },
    {}
  );
  ...
  return stages.map((stage) => {
    ...
    const conversion = prevCount > 0 && stage !== "applied" ? Number.parseFloat((count / prevCount * 100).toFixed(1)) : 100;
    prevCount = stage !== "rejected" ? count : prevCount;
    return {
      stage,
      label: labels[stage],
      count,
      percent: total > 0 ? Number.parseFloat((count / total * 100).toFixed(1)) : 0,
      conversion
    };
  });
}
function getPackageDistribution() {
  ...
  const packages = mockStudents.filter((s2) => s2.package_lpa).map((s2) => s2.package_lpa);
  return ranges.map((r) => ({
    ...r,
    count: packages.filter((p2) => p2 >= r.min && p2 < r.max).length
  }));
}
function getCGPAPlacementData() {
  const ranges = [
    { range: "<6", min: 0, max: 6 },
  ...
  return ranges.map((r) => {
    const students = mockStudents.filter(
      (s2) => s2.cgpa >= r.min && s2.cgpa < r.max
    );
    const placed = students.filter((s2) => s2.placement_status === "placed");
    return {
      range: r.range,
      total: students.length,
      placed: placed.length,
      rate: students.length ? Number.parseFloat(
        (placed.length / students.length * 100).toFixed(1)
      ) : 0
    };
  });
}
function getAtRiskStudents(limit = 15) {
  return mockStudents.filter((s2) => s2.placement_status !== "placed").map((s2) => ({
    ...s2,
    risk_score: 100 - (s2.cgpa * 5 + s2.aptitude_score * 0.25 + s2.programming_score * 0.25)
  })).sort((a2, b2) => b2.risk_score - a2.risk_score).slice(0, limit);
}
function getSectionStats(department) {
  return SECTIONS.map((section) => {
    const students = mockStudents.filter(
      (s2) => s2.department === department && s2.section === section
    );
    const placed = students.filter((s2) => s2.placement_status === "placed");
    return {
      section,
      ...
      avg_cgpa: students.length ? Number.parseFloat(
        (students.reduce((a2, s2) => a2 + s2.cgpa, 0) / students.length).toFixed(2)
      ) : 0,
      students
    };
  });
...
export {
  Bar as $,
  Animate as A,
  getCateCoordinateOfLine as B,
  Curve as C,
  Dot as D,
  generateCategoricalChart as E,
  ...
  mockStudents as M,
  mockCompanies as N,
  mockApplications as O,
  CartesianGrid as P,
  Legend as Q,
  ResponsiveContainer as R,
  Shape as S,
  Text as T,
  monthlyTrendData as U,
  getDeptStats as V,
  getBatchStats as W,
  XAxis as X,
  YAxis as Y,
  ...
  _baseExtremum as _,
  _baseGt as a,
  getActivityFeed as a0,
  getPipelineFunnel as a1,
  ChevronDown as a2,
  DEPARTMENTS as a3,
  SCHOOLS as a4,
  getCGPAPlacementData as a5,
  getAtRiskStudents as a6,
  getSectionStats as a7,
  getTopCompanies as a8,
  getPackageDistribution as a9,
  getSkillGapData as aa,
  _baseIteratee as b,
  _baseLt as c,
